...
class SecurityModel extends CI_Model
{
  ...
  public function hasUserdataKeyGuard($key, $ajax = FALSE)
  {
    if ($this->session->userdata($key) == NULL) {
      ...
      redirect($this->session->userdata('login')['nama_controller']);
    }
  }

  public function certainUserGuard($userIds = array(), $ajax = FALSE)
  {
    if (!in_array($this->session->userdata('login')['id_user'], $userIds)) {
      if ($ajax) throw new UserException('Kamu tidak berhak mengakses resource ini', UNAUTHORIZED_CODE);
      redirect($this->session->userdata('login')['nama_controller']);
    }
  }
  ...
  {
    if (strtolower($this->session->userdata('login')['nama_role']) != $role) {
      if ($ajax) throw new UserException('Kamu tidak berhak mengakses resource ini', UNAUTHORIZED_CODE);
      redirect(base_url());
    }
  }
  ...
  {
    if (!in_array(strtolower($this->session->userdata('login')['nama_role']), $roles)) {
      if ($ajax) throw new UserException('Kamu tidak berhak mengakses resource ini', UNAUTHORIZED_CODE);
      redirect($this->session->userdata('login')['nama_controller']);
    }
  }
